...
function computeReadiness(trip, hasItinerary, itemCount, health, issues, reservations, budget) {
  const areas = [];
  if (hasItinerary) {
    areas.push({
      key: "itinerary",
      label: "Itinerary",
      status: "ready",
      detail: `${itemCount} item(s) scheduled`
    });
  } else {
    areas.push({
      key: "itinerary",
  ...
  if (health) {
    ...
    areas.push({
      ...
      label: "Trip Health",
      status: "unknown",
      detail: "Health not yet calculated"
    });
  }
  ...
  if (criticalCount > 0) {
    areas.push({
      ...
      label: "Issues",
      status: "critical",
      detail: `${criticalCount} critical issue(s)`
    });
  } else if (highCount > 0) {
    areas.push({
      key: "issues",
      label: "Issues",
      status: "warning",
      detail: `${highCount} high-priority issue(s)`
    });
    ...
    areas.push({
      ...
      label: "Issues",
      status: "info",
      detail: `${issues.length} minor issue(s)`
    });
  } else {
    areas.push({
      key: "issues",
  ...
  if (reservations.length === 0) {
    ...
    areas.push({
      ...
      label: "Reservations",
      status: "warning",
      detail: `${needsReviewRes} reservation(s) need review`
    });
  } else {
    areas.push({
      key: "reservations",
      label: "Reservations",
      status: "ready",
      detail: `${confirmedRes}/${reservations.length} confirmed`
    });
  }
  if (budget) {
    const bStatus = budget.budget_status;
    const uiStatus = bStatus === "OVER_BUDGET" ? "warning" : bStatus === "ON_TRACK" ? "good" : "info";
    areas.push({
      key: "budget",
      label: "Budget",
      status: uiStatus,
      detail: `Status: ${bStatus}`
    });
  } else {
    ...
    areas.push({
      ...
      label: "Budget",
      status: "unknown",
      detail: "No budget target is recorded for this trip"
    });
  }
  ...
  if ([
    "critical",
    "poor"
  ].includes(worstStatus)) overall = "NEEDS_ATTENTION";
  else if ([
    "warning",
    "fair"
  ].includes(worstStatus)) overall = "MOSTLY_READY";
  else if ([
    "good",
    "ready"
  ].includes(worstStatus)) overall = "READY";
...
Deno.serve(async (req)=>{
  ...
  let itemRows = [];
  try {
    const { data, error: itemsErr } = await supabase.from("itinerary_items").select("*").eq("trip_id", trip_id).order("date", {
      ascending: true
    }).order("start_time", {
      ascending: true
    });
  ...
  const [reservationsResult, healthResult, frictionResult, issuesResult, budgetResult, assemblyResult, readinessResult, readinessItemsResult, docsResult, infoResult, impactsResult, alertsResult] = await Promise.allSettled([
    ...
    supabase.from("trip_health_analyses").select("*").eq("trip_id", trip_id).eq("status", "ready").order("analyzed_at", {
      ascending: false
    }).limit(1).maybeSingle(),
    supabase.from("daily_friction_scores").select("*").eq("trip_id", trip_id).eq("status", "ready").order("calculated_at", {
      ascending: false
    }).limit(1).maybeSingle(),
    supabase.from("trip_issues").select("*").eq("trip_id", trip_id).in("status", [
      "OPEN",
      "ACKNOWLEDGED"
    ]).order("created_at", {
      ascending: false
    ...
    supabase.from("budget_analyses").select("*").eq("trip_id", trip_id).eq("status", "ready").order("analyzed_at", {
      ascending: false
    }).limit(1).maybeSingle(),
    ...
    supabase.from("readiness_items").select("title, description, severity, category, recommendation, priority").eq("trip_id", trip_id).in("status", [
      "OPEN"
    ]).order("priority", {
      ascending: true
    }).limit(10),
    supabase.from("secure_documents").select("id, document_type, document_name, expiration_date, status, scope, trip_id, reservation_id, extraction_confidence").eq("user_id", userId).eq("is_active", true).order("created_at", {
      ascending: false
    }).limit(20),
    ...
    supabase.from("trip_impacts").select("id, impact_type, impact_level, affected_entity_type, affected_entity_id, explanation, confidence, status, created_at").eq("trip_id", trip_id).eq("status", "ACTIVE").order("impact_level", {
      ascending: true
    }).limit(10),
    supabase.from("travel_alerts").select("id, alert_type, priority, urgency, title, summary, confidence, time_to_impact, status, created_at").eq("trip_id", trip_id).eq("status", "ACTIVE").order("created_at", {
      ascending: false
    }).limit(5)
  ]);
  ...
  const friction = frictionResult.status === "fulfilled" ? frictionResult.value.data ?? null : null;
  const rawIssues = issuesResult.status === "fulfilled" ? issuesResult.value.data ?? [] : [];
  const budget = budgetResult.status === "fulfilled" ? budgetResult.value.data ?? null : null;
  const assembly = assemblyResult.status === "fulfilled" ? assemblyResult.value.data ?? null : null;
  const preReadiness = readinessResult.status === "fulfilled" ? readinessResult.value.data ?? null : null;
  const readinessItems = readinessItemsResult.status === "fulfilled" ? readinessItemsResult.value.data ?? [] : [];
  const vaultDocs = docsResult.status === "fulfilled" ? docsResult.value.data ?? [] : [];
  const vaultInfo = infoResult.status === "fulfilled" ? infoResult.value.data ?? [] : [];
  const rawImpacts = impactsResult.status === "fulfilled" ? impactsResult.value.data ?? [] : [];
  ...
  const highestImpactLevel = rawImpacts.length > 0 ? rawImpacts.reduce((best, cur)=>{
    const curLevel = cur.impact_level;
  ...
  const sortedAlerts = [
    ...
    const pDiff = (ALERT_PRIORITY_ORDER[a.priority] ?? 99) - (ALERT_PRIORITY_ORDER[b.priority] ?? 99);
  ...
  if (friction?.scores && Array.isArray(friction.scores)) {
    for (const s of friction.scores){
      if (s.day_number != null) frictionByDay[s.day_number] = s;
    }
  }
  const issuesByDay = {};
  for (const issue of issues){
    const dn = issue.day_number;
    if (dn != null) {
  ...
  for (const it of itemRows){
    const dn = dayNumberFor(it.date, trip.start_date);
    if (dn == null) {
  ...
  const days = dayNumbers.map((dn)=>{
    ...
    const dayDate = dayItems[0]?.date ?? null;
    ...
    return {
      ...
      friction: {
        score: frictionDay?.friction_score ?? null,
        status: frictionDay?.friction_status ?? null,
        top_factors: frictionDay?.top_factors ?? [],
        explanation: frictionDay?.explanation ?? null,
        confidence: frictionDay?.confidence ?? null
      },
      issues: dayIssues.map((i)=>({
          issue_id: i.issue_id ?? i.id,
          severity: i.severity,
          category: i.category,
          title: i.title,
          short_description: i.short_description,
          recommended_action: i.recommended_action,
          fixable_by: i.fixable_by
        })),
      reservations: dayReservations.map((r)=>({
          ...
          reservation_status: r.reservation_status ?? "UNKNOWN",
  ...
  const frictionSummary = {
    ...
    high_friction_days: friction?.high_friction_days ?? [],
    data_available: frictionAvailable
  };
  ...
  const healthSection = {
    overall_score: health?.health_score ?? null,
    status: health?.health_status ?? null,
    confidence: null,
    schedule_score: health?.schedule_score ?? null,
    geography_score: health?.geography_score ?? null,
    pace_score: health?.pace_score ?? null,
    budget_score: health?.budget_score ?? null,
    completeness_score: health?.completeness_score ?? null,
    primary_factors: [],
    overall_assessment: health?.overall_assessment ?? null,
    calculated_at: health?.analyzed_at ?? null,
    data_available: healthAvailable
  };
  const criticalCount = issues.filter((i)=>i.severity === "CRITICAL").length;
  const highCount = issues.filter((i)=>i.severity === "HIGH").length;
  const issuesSection = {
    ...
    items: issues.map((i)=>({
        ...
        fixable_by: i.fixable_by,
        confidence: i.confidence,
        change_plan_prompt: i.change_plan_prompt ?? null,
        day_number: i.day_number ?? null
      }))
  };
  ...
  const budgetSection = {
    target: null,
    target_note: "no budget target is recorded on the trip",
    currency: trip.base_currency ?? NOT_SPECIFIED,
    status: budget?.budget_status ?? null,
    total_projected: budget?.total_projected_cost ?? null,
    remaining: budget?.remaining_budget ?? null,
    cost_drivers: budget?.cost_drivers ?? [],
    data_available: budgetAvailable,
    calculated_at: budget?.analyzed_at ?? null
  };
  const assemblyAvailable = !!assembly;
  const tripAssemblySection = {
    assembly_status: assembly?.assembly_status ?? null,
    confidence: assembly?.confidence ?? null,
    assembly_notes: assembly?.assembly_notes ?? null,
    destination_count: assembly?.destination_count ?? 0,
    destinations: assembly?.destinations ?? [],
  ...
  const topReadinessItems = readinessItems.slice(0, 5).map((item)=>({
      title: item.title,
  ...
  const preTripReadinessSection = {
    overall_status: preReadiness?.overall_status ?? null,
    summary_message: preReadiness?.summary_message ?? null,
    days_until_trip: preReadiness?.days_until_trip ?? null,
    trip_phase: preReadiness?.trip_phase ?? null,
    open_item_count: preReadiness?.open_item_count ?? 0,
    critical_item_count: preReadiness?.critical_item_count ?? 0,
    top_items: topReadinessItems,
    data_available: readinessDataAvailable
  };
  ...
  const dataQuality = {
    ...
    issues_available: issuesAvailable,
    reservations_available: reservationsAvailable,
    budget_available: budgetAvailable,
    pace_available: false,
    geo_available: false,
    trip_assembly_available: assemblyAvailable,
    pre_trip_readiness_available: readinessDataAvailable,
  ...
  const tripSection = {
    ...
    duration_days: durationDays(trip.start_date, trip.end_date),
    traveler_count: NOT_SPECIFIED,
    traveler_type: null,
    travel_style: null,
    budget_target: null,
    currency: trip.base_currency ?? NOT_SPECIFIED,
  ...
  const itinerarySection = {
    ...
    unscheduled_item_count: unscheduledItems.length,
    days
  };
  const response = {
    context_version: "2.0",
    ...
    trip_assembly: tripAssemblySection,
    pre_trip_readiness: preTripReadinessSection,
    readiness,
    friction_summary: frictionSummary,
    pace_summary: paceSummary,
    geo_summary: geoSummary,
    document_vault: documentVaultSection,
    trip_impacts: tripImpactsSection,
    travel_alerts: travelAlertsSection,
